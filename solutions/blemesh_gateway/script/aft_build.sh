...
cp "${BASE_PWD}/config.yaml" $MK_GENERATED_PATH/data/
cp "${BASE_PWD}/kp" $MK_GENERATED_PATH/data/
...
$BOARD_DIR\\pack\\pack.exe -d 0 -r ${RTOS_IMG} -o ${OUT_DIR} -m ${MK_GENERATED_IMGS_PATH} -c ${CONFIG_YAML_FILE}
