...
def get_mem_info(map_file):
    ...
    with open(map_file, 'r') as f:
        ...
        rom_config_text = re.findall(r'\w+\s+(0x\w+)\s+(0x\w+)\s+(?:xr|r)\r?\n',mem_config_text)
        ...
        ram_config_text = re.findall(r'\w+\s+(0x\w+)\s+(0x\w+)\s+(?:xrw|rw)\r?\n',mem_config_text)
        if (len(ram_config_text)+len(rom_config_text)) == 0:
            ram_config_text = re.findall(r'DRAM\s+(0x\w+)\s+(0x\w+)\r?\n',mem_config_text)
        ...
        for module in modules:
            ...
            if(module == '*fill*'):
                sections = map(lambda arg : {'address':int(arg[0], 16), 'size':int(arg[1], 16)}, re.findall(r'\*fill\*[ \t]+(0x\w+)[ \t]+(0x\w+)[ \t]+\r?\n', mem_map))
            else:
                sections = map(lambda arg : {'address':int(arg[0], 16), 'size':int(arg[1], 16)}, re.findall(r'(0x\w+)[ \t]+(0x\w+)[ \t]+.+[/\\]'+module+r'(\(.+\.o\))?\r?\n', mem_map))
            if(not sections):
                continue
            ...
            map_lines.append(r'| %-40s | %-8d  | %-8d |'%(re.sub('\.[ao]','',module)[:40],rom_size,ram_size))
