...
def read_platform_info():
    ...
    # debug_print(top)
    plat_file = os.path.join(pack_tools_dir, "platform.txt")
    debug_print(plat_file)
    if not os.path.exists(plat_file):
        print("platformf file not exists!")
        sys_exit(1)
...
def copy_files():
    ...
    if os.path.exists(fs_img):
        dst = os.path.join(image_dir, "littlefs.fex")
        shutil.copy(fs_img, dst)
    dst = os.path.join(image_dir , "sys_partition_nor.fex")
...
def updarte_mbr(storage, udisk_size, full_size):
    ...
    if not os.path.exists(sys_partition_file):
        print('not exists %s file' % sys_partition_file)
        sys_exit(1)
    ...
    if isWindows():
        exec_cmd("convert_gpt.exe", "-s",  mbr_source_file, "-o", gpt_file, "-l 96 -u 8")
    else:
        exec_cmd("./mbr_convert_to_gpt", "-s",  mbr_source_file, "-o", gpt_file, "-l 96 -u 8")
...
def create_img(part_info):
        ...
        for line in f:
            if re.search('filename[ ]+=[ ]".*"', line):
                filename = re.sub('"', '', line.split(',')[
                                  0].split('=')[1]).strip()
                filedir = os.path.join(os.path.relpath(image_dir), filename)
    ...
    if not os.path.exists(image_src):
        ...
        sys_exit(1)
    image_dst = os.path.join(out_dir, "yoc_rtos_" + platformf['nor_volume'] + "M.img")
...
if __name__ == "__main__":
    ...
    copy_files()
    copy_pack_tools()
    do_update(platformf['torage_type'])
    if platformf['torage_type'] == "nor":
        part_info = perpare_for_nor()
    do_imageszip()
    do_finish(platformf['torage_type'], part_info)
    remove_pack_tools()
