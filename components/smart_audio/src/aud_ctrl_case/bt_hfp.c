...
static yoc_bd_addr_t peer_addr = {0xb4, 0xe6, 0x2d, 0xeb, 0x09, 0x93};
/* callback for HF_CLIENT */
static void bt_app_hf_client_cb(yoc_hf_client_cb_event_t event, yoc_hf_client_cb_param_t *param)
{
    if (event <= YOC_HF_CLIENT_RING_IND_EVT) {
        LOGD(TAG, "APP HFP event: %s", c_hf_evt_str[event]);
    } else {
        LOGE(TAG, "APP HFP invalid event %d", event);
    }
    switch (event) {
    case YOC_HF_CLIENT_CONNECTION_STATE_EVT: {
        ...
        memcpy(peer_addr, param->conn_stat.remote_bda, BT_BD_ADDR_LEN);
        char bdAddr[YOC_APP_BDADDR_MAX_LEN];

        if (param->conn_stat.state == YOC_HF_CLIENT_CONNECTION_STATE_CONNECTED ||
            param->conn_stat.state == YOC_HF_CLIENT_CONNECTION_STATE_SLC_CONNECTED) {
            yoc_app_bt_hfp_connect_flag = 1;
        } else {
            yoc_app_bt_hfp_connect_flag = 0;
        }

        mac_bin_to_str(bdAddr, param->conn_stat.remote_bda);
        ...
        if (yoc_app_bt_hfp_disconnect_flag == 1 &&
            param->conn_stat.state == YOC_HF_CLIENT_CONNECTION_STATE_DISCONNECTED) {
    ...
    case YOC_HF_CLIENT_AUDIO_STATE_EVT: {
        ...
        if (param->audio_stat.state == YOC_HF_CLIENT_AUDIO_STATE_CONNECTED ||
            param->audio_stat.state == YOC_HF_CLIENT_AUDIO_STATE_CONNECTED_MSBC) {
            start_pcm_send();
        } else if (param->audio_stat.state == YOC_HF_CLIENT_AUDIO_STATE_DISCONNECTED) {
            stop_pcm_send();
        }
        break;
    }
    case YOC_HF_CLIENT_BVRA_EVT: {
        LOGI(TAG, "--VR state %s", c_vr_state_str[param->bvra.value]);
        break;
    }
    case YOC_HF_CLIENT_CIND_SERVICE_AVAILABILITY_EVT: {
        LOGI(TAG, "--NETWORK STATE %s",
             c_service_availability_status_str[param->service_availability.status]);
        break;
    }
    case YOC_HF_CLIENT_CIND_ROAMING_STATUS_EVT: {
        LOGI(TAG, "--ROAMING: %s", c_roaming_status_str[param->roaming.status]);
        break;
    }
    case YOC_HF_CLIENT_CIND_SIGNAL_STRENGTH_EVT: {
        LOGI(TAG, "-- signal strength: %d", param->signal_strength.value);
        break;
    }
    case YOC_HF_CLIENT_CIND_BATTERY_LEVEL_EVT: {
        LOGI(TAG, "--battery level %d", param->battery_level.value);
        break;
    }
    case YOC_HF_CLIENT_COPS_CURRENT_OPERATOR_EVT: {
        LOGI(TAG, "--operator name: %s", param->cops.name);
        break;
    }
    case YOC_HF_CLIENT_CIND_CALL_EVT: {
        LOGI(TAG, "--Call indicator %s", c_call_str[param->call.status]);

        if (param->call.status == YOC_HF_CALL_STATUS_NO_CALLS) {
            g_hfp_call_status = CALL_NO_PROGRESS;
            if (ctrl_bt_hfp.callback) {
                ctrl_bt_hfp.callback(SMTAUDIO_BT_HFP, SMTAUDIO_PLAYER_EVENT_STOP);
            }
        } else if (param->call.status == YOC_HF_CALL_STATUS_CALL_IN_PROGRESS) {
    ...
    case YOC_HF_CLIENT_CIND_CALL_SETUP_EVT: {
        LOGI(TAG, "--Call setup indicator %s", c_call_setup_str[param->call_setup.status]);

        if (param->call_setup.status == YOC_HF_CALL_SETUP_STATUS_INCOMING) {
            g_hfp_call_status = CALL_INCOMING;
            if (ctrl_bt_hfp.callback) {
                ctrl_bt_hfp.callback(SMTAUDIO_BT_HFP, SMTAUDIO_PLAYER_EVENT_START);
            }
        } else if (param->call_setup.status == YOC_HF_CALL_SETUP_STATUS_OUTGOING_DIALING) {
            g_hfp_call_status = CALL_OUTGOING_DIALING;
            if (ctrl_bt_hfp.callback) {
                ctrl_bt_hfp.callback(SMTAUDIO_BT_HFP, SMTAUDIO_PLAYER_EVENT_START);
            }
        } else if (param->call_setup.status == YOC_HF_CALL_SETUP_STATUS_NONE) {
    ...
    case YOC_HF_CLIENT_VOLUME_CONTROL_EVT: {
...
int32_t yoc_app_bt_hfp_disconnect(const char *mac)
{
    ...
    int ret = yoc_hf_client_disconnect(addr);
...
int32_t yoc_app_bt_hfp_interrupt_cur_conn(void)
{
    ...
    int ret = yoc_hf_client_disconnect(peer_addr);
...
int32_t yoc_app_bt_hfp_connect_audio(const char *mac)
{
    ...
    yoc_hf_client_connect_audio(addr);
    return 0;
}
...
int32_t yoc_app_bt_hfp_disconnect_audio(const char *mac)
{
    ...
    yoc_hf_client_disconnect_audio(addr);
    return 0;
}
...
int32_t yoc_app_bt_hfp_send_command(YOC_APP_BT_HFP_COMMAND_T command)
{
    ...
    case YOC_APP_BT_HFP_COMMAND_TYPE_REJECT_INCOMING:
        yoc_hf_client_send_chld_cmd(YOC_HF_CHLD_TYPE_REL, 0);
        break;
    case YOC_APP_BT_HFP_COMMAND_TYPE_TERMINATE_ACTIVE_ACCEPT_INCOMING:
        yoc_hf_client_send_chld_cmd(YOC_HF_CHLD_TYPE_REL_ACC, 0);
        break;
    case YOC_APP_BT_HFP_COMMAND_TYPE_HOLD_ACTIVE_ACCEPT_INCOMING:
        yoc_hf_client_send_chld_cmd(YOC_HF_CHLD_TYPE_HOLD_ACC, 0);
        break;
        /* Two-way calling */
    case YOC_APP_BT_HFP_COMMAND_TYPE_ANSWER:
        if (g_hfp_call_status != CALL_IN_PROGRESS) {
            yoc_hf_client_answer_call();
        }
        break;
    case YOC_APP_BT_HFP_COMMAND_TYPE_TERMINATE:
        yoc_hf_client_reject_call();
        break;
    default:
        break;
    }
...
int32_t yoc_app_bt_hfp_dial(const char *number)
{
    yoc_hf_client_dial(number);
...
{
    yoc_hf_client_volume_update(type, volume);
...
static int32_t yoc_app_bt_hfp_init(void)
{
    ...
    yoc_hf_client_register_callback(bt_app_hf_client_cb);
    yoc_hf_client_init();
...
static int bt_hfp_vol_set(int vol)
{
    yoc_app_bt_hfp_volume_update(YOC_HF_VOLUME_CONTROL_TARGET_SPK, vol * 15 / 100);
    return 0;
}
...
uint32_t smtaudio_register_bt_hfp(uint8_t min_vol, uint8_t *aef_conf, size_t aef_conf_size, float speed, int resample)
{
    uint32_t id;
    id = smtaudio_ops_register(&ctrl_bt_hfp);
    ctrl_bt_hfp.init();
