...
smtaudio_ops_node_t ctrl_local_play = {
    .name     = "local_play",
    .prio     = 1,
...
static int local_play_vol_up(int vol)
{
    ...
    aui_player_vol_adjust(SMTAUDIO_LOCAL_PLAY, vol);
...
uint32_t smtaudio_register_local_play(uint8_t min_vol, uint8_t *aef_conf, size_t aef_conf_size, float speed, int resample)
{
    int      vol;
    uint32_t id;

    id = smtaudio_ops_register(&ctrl_local_play);
    ctrl_local_play.init();

    media_init(aef_conf, aef_conf_size, speed, resample);

    aui_player_key_config(media_dec_key_cb);
    aui_player_set_minvol(SMTAUDIO_LOCAL_PLAY, (min_vol <= 0) ? 20 : min_vol);

    aos_kv_getint(VOLUME_SAVE_KV_NAME, &vol);
    ctrl_local_play.vol_set(((vol == 0) ? 30 : vol));
