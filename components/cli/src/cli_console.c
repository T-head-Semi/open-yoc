...
#ifdef CLI_CONSOLE_USER_INFO_POS
#define CONSOLE_USER_INFO_ID (CLI_CONSOLE_USER_INFO_POS)
#else
#define CONSOLE_USER_INFO_ID (2)
#endif
...
{
    return (cli_console *) aos_task_getspecific(CONSOLE_USER_INFO_ID);
}
...
{
    return aos_task_setspecific(CONSOLE_USER_INFO_ID, NULL);
}
...
int cli_task_set_console(void *console)
{
    ...
    if (mul_console_init_flag == 1) {
        cli_console_shared_sem_take(&console_sem);
        ret = aos_task_setspecific(CONSOLE_USER_INFO_ID, console);
        cli_console_shared_sem_post(&console_sem);
    } else {
        ret = aos_task_setspecific(CONSOLE_USER_INFO_ID, console);
    }
